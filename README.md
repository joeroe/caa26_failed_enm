# Can ecological models predict the occurrence of species in the archaeological record? Can I?

This research compendium contains the data and source code for a paper presented at the [Computer Applications and Quantitative Methods in Archaeology (CAA) annual conference in Vienna, 31 March – 4th April 2026](https://2026.caaconference.org/).

The slides use [remark.js](https://remarkjs.com/#1) and are generated using [Quarto](https://quarto.org).
The source for the slides is `caa26_failed_enm.qmd`.
You will need to install the R packages listed at the top of the document to be able to preview or render it.

## How to view the slides

### Locally

`caa26_failed_enm.html` is a self-contained HTML document containing the slides.
To view them, download or clone it and open it in a web browser.

### Online

The slides can also be viewed online at <https://joeroe.io/caa26_failed_enm/caa26_failed_enm.html>

Press `s` in the presentation window and see the speaker notes.

